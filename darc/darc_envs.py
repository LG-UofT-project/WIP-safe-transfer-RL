...
class InvertedPendulumModifiedEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    """ Variant of the Inverted Pendulum environment from Mujoco""" 
    def __init__(self, use_heavy = False):
        utils.EzPickle.__init__(self)
        if use_heavy:
            xml_file_path = os.getcwd()+'/xmls/inverted_pendulum_mod.xml'
        else:
            xml_file_path = os.getcwd()+'/xmls/inverted_pendulum.xml'
        mujoco_env.MujocoEnv.__init__(self, xml_file_path , 2)
        self.observation_space = gym.spaces.Box(
        low=self.observation_space.low,
        high=self.observation_space.high,
        dtype=np.float32,
...
def get_inverted_pendulum_env(mode):
  if mode == "sim":
    env = InvertedPendulumModifiedEnv(use_heavy = False)
  else:
    assert mode == "real"
    env = InvertedPendulumModifiedEnv(use_heavy= True)
  env = suite_gym.wrap_env(env, max_episode_steps=1000)
  return tf_py_environment.TFPyEnvironment(env)
